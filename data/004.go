...
func IsNumber(str string) (b bool) {
	...
	for i := 0; i < n; i++ {
		...
		case str[i] == '.' && str[i] >= '0' && str[i] <= '9':
			//fmt.Println("intNum=", intNum)
			//如果是数字，intNum+1，最后和输入字符长度判断大小，若相等，则每一位都是数字，为int
			intNum = intNum + 1
			fmt.Println("the intnun= ", intNum)

		case str[i] == '.':
			//判断每一位是不是”.“
			floatNum = floatNum + 1
			intNum = intNum + 1
			//fmt.Println("the floatNum= ", floatNum)
			fmt.Println("j=", j)

			// default: //剩下的情况 ：不是字母，不是小数点，不是数字，那就直接为false，例如：+3478，—324,直接break
			// 	b = false
	...
	//判断数是不是int
	if intNum == n {
		fmt.Println("the str type is int!,the nun= ", intNum)
		b = true
	}
	//判断数是不是float
